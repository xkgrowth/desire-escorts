...
function HeartDefs({
  ...
  return (
    <defs>
      {/* Base gold gradient — right half */}
      <linearGradient id={`${id}-gr`} x1="25" y1="0" x2="17" y2="32.5" gradientUnits="userSpaceOnUse">
        <stop stopColor="#F0D074" />
        <stop offset="0.57" stopColor="#F2DE9B" />
      </linearGradient>

      {/* Base gold gradient — left half */}
      <linearGradient id={`${id}-gl`} x1="6.03" y1="0.672" x2="16.47" y2="30.224" gradientUnits="userSpaceOnUse">
        <stop stopColor="#F0D074" />
        <stop offset="0.567" stopColor="#F2DE9B" />
      </linearGradient>

      {/* Shimmer sweep — used by both static (one-pass) and animated (looping) */}
      <linearGradient id={`${id}-sh`} x1="-10" y1="0" x2="44" y2="30" gradientUnits="userSpaceOnUse">
      ...
      {/* Bevel border gradient — bright top-left, fades to transparent bottom-right */}
      <linearGradient id={`${id}-bevel`} x1="0" y1="0" x2="34" y2="30" gradientUnits="userSpaceOnUse">
      ...
      {/* Outer ring gradient — subtle gold rim */}
      <linearGradient id={`${id}-ring`} x1="0" y1="0" x2="34" y2="30" gradientUnits="userSpaceOnUse">
...
function HeartBody({ id }: { id: string }) {
  return (
    <>
      ...
      {/* Shimmer overlay */}
      <rect x="0" y="0" width="34" height="30" fill={`url(#${id}-sh)`} clipPath={`url(#${id}-cr)`} />
      <rect x="0" y="0" width="34" height="30" fill={`url(#${id}-sh)`} clipPath={`url(#${id}-cl)`} />

      {/* Inner bevel highlight — 0.7px stroke inset */}
      <path d={RIGHT} fill="none" stroke={`url(#${id}-bevel)`} strokeWidth="0.7" />
      <path d={LEFT}  fill="none" stroke={`url(#${id}-bevel)`} strokeWidth="0.7" />

      {/* Outer ring border */}
      <path d={RIGHT} fill="none" stroke={`url(#${id}-ring)`} strokeWidth="0.45" />
      <path d={LEFT}  fill="none" stroke={`url(#${id}-ring)`} strokeWidth="0.45" />
    </>
  )
}
